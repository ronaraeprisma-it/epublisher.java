...
, playlists AS (
	SELECT
			...
			, br.duration
			, pl.settingsdifferentthanpublished
			, pl.ancestorplaylistid
	FROM
			screengroups sgs
	...
	WHERE
			...
			AND (pl.state IS NULL OR pl.state != 'unpublished')
...
,  playlistTime AS (
	SELECT DISTINCT 
		p.*, pt.starthour,pt.startminute,pt.endhour,pt.endminute,
			(CASE 
				WHEN (  (select  days from playtime_days where days = EXTRACT(DOW FROM current_timestamp) and playtime_id = pt.id) >= 0
				AND (
	...
	FROM
			playlists p
			INNER JOIN playTime pt ON (p.playlistId = pt.playlist_id)
			INNER JOIN playtime_days ptd ON (pt.id = ptd.playtime_id)
	WHERE
